...
class _DifferentialExpressionTestCont(_DifferentialExpressionTestSingle):
    ...
    def _filter_genes_int(self, genes: list):
        ...
        :return: Filtered list of genes
        """
        genes_found = np.array([idx for idx, x in enumerate(genes) if x < self.x.shape[1]])
        if len(genes_found) < len(genes):
            logger.info("did not find some genes, omitting")
            genes = genes[genes_found]
        return genes
    ...
    def _spline_par_loc_idx(self, intercept=True):
        ...
        """
        par_loc_names = self._model_estim.model_container.loc_names
    ...
    def _continuous_model(self, idx, non_numeric=False):
        ...
        if non_numeric:
            mu = np.matmul(self._model_estim.model_container.design_loc,
                           self._model_estim.model_container.theta_location_constrained[:, idx])
            if self._size_factors is not None:
                mu = mu + self._model_estim.model_container.size_factors
        else:
            idx_basis = self._spline_par_loc_idx(intercept=True)
            mu = np.matmul(self._model_estim.model_container.design_loc[:, idx_basis],
                           self._model_estim.model_container.theta_location_constrained[idx_basis, :][:, idx])
    ...
    def _continuous_interpolation(self, idx):
        ...
        idx_basis = self._spline_par_loc_idx(intercept=True)
        a = self._model_estim.model_container.theta_location_constrained[idx_basis, :]
    ...
    def plot_genes(
        ...
        for i, g in enumerate(gene_idx):
            ...
            if isinstance(y, scipy.sparse.spmatrix) or isinstance(y, sparse.COO):
                ...
                if self._model_estim.model_container.size_factors is not None:
                    y = y / self._model_estim.model_container.size_factors
            ...
            if scalings is not None:
                yhat = np.vstack([
                    ...
                    [
                        yhat * np.expand_dims(
                            np.exp(self._model_estim.a_var[self._model_estim.model_container.loc_names.index(x), g]),
                            axis=0
                        )
