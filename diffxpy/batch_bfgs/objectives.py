...
def objective(theta, x, design_loc, design_scale, lib_size, batch_size=100):
    if batch_size is None:
        J = np.sum(objective_ll(x=x, 
            theta_mu=np.asarray(theta)[:design_loc.shape[1]], theta_disp=np.asarray(theta)[design_loc.shape[1]:], 
            design_loc=design_loc, design_scale=design_scale, lib_size=lib_size))
    else:
        batch_idx = numpy.random.randint(low=0, high=x.shape[0], size=(batch_size))
        J = np.sum(objective_ll(x=x[batch_idx], 
            theta_mu=np.asarray(theta)[:design_loc.shape[1]], theta_disp=np.asarray(theta)[design_loc.shape[1]:], 
            design_loc=design_loc[batch_idx,:], design_scale=design_scale[batch_idx,:], lib_size=lib_size[batch_idx]))
    return J